...
func TestArticleContentLines_RendersCommonArticleElements(t *testing.T) {
	entry := feedbin.Entry{
		Content: `<article>
			<h1>Main Title</h1>
			<h2>Subtitle</h2>
...
func TestArticleContentLines_PostProcessingWikipediaStopsAtReferences(t *testing.T) {
	entry := feedbin.Entry{
	...
	got := stripANSI(strings.Join(articleContentLines(entry, 80), "\n"))
...
func TestArticleContentLines_PostProcessingWiredRemovesPromoTail(t *testing.T) {
	entry := feedbin.Entry{
...
func TestModelUpdate_PreferenceTogglesPersist(t *testing.T) {
	...
	updated, cmd = model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'N'}})
	if cmd == nil {
		t.Fatal("expected preference save command after numbering toggle")
	}
	_ = cmd()

	if len(saved) != 5 {
		t.Fatalf("expected 5 persisted preference snapshots, got %d", len(saved))
	}
	if !saved[0].Compact {
	...
	if !saved[4].ShowNumbers {
		t.Fatalf("expected show-numbers true after fifth save, got %+v", saved[4])
	}
}

func TestModelUpdate_InlineImagePreviewSuccess(t *testing.T) {
	t.Setenv("FEEDBIN_INLINE_IMAGE_PREVIEW", "1")
	m := NewModel(nil, []feedbin.Entry{{
		ID:          1,
		Title:       "Entry",
